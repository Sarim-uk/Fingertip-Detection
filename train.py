...
# Compile
adam = Adam(learning_rate=1e-5, beta_1=0.9, beta_2=0.999, epsilon=1e-10)
...
checkpoints = ModelCheckpoint('weights/weights_{epoch:03d}.weights.h5', save_weights_only=True, save_freq=1)
history = model.fit(train_gen, steps_per_epoch=training_steps_per_epoch, epochs=epochs, verbose=1,
                    ...
                    callbacks=[checkpoints], shuffle=True)
...
# Combine loss functions into a dictionary
loss_functions = {"prob_output": loss_function_1, "pos_output": loss_function_2}
model.compile(optimizer=adam, loss=loss_functions, metrics=None)
